<?php
    ...
    * */
    function index_multiple_html_files($path_directory, $array_empty_words, $séparateurs){
        $html_file_list = get_html_files_paths($path_directory);
        
        $files = [];
        foreach ($html_file_list as $index => $html_file_path) {
            $files[] = index_one_html_file($html_file_path, $array_empty_words, $séparateurs);
        }
        return $files;
    }
    ...
    * */
    function index_one_html_file($html_file_path,$array_empty_words,$separator){
        # lire le fichier html
        $word = file_get_contents($html_file_path);
        ...
        return convert_array_to_doc(
            ...
            $html_file_path, 
            $token->occurrences,
            $token->totale_occurrences,
            get_metas_description_and_keywords($html_file_path,'description'),
            get_metas_description_and_keywords($html_file_path,'keywords'),
        );
    }
    ...
    */
    function explode_html_file($path_file, $text, $separator, $array_empty_words){
        ...
        $array_word_head = explode_text_in_html_file($separator, $array_empty_words, $head);
        $occurrences_head_word = array_count_values($array_word_head);
        // <body>
        $body = get_html_body($text);
        $tab_word_body = explode_text_in_html_file($separator, $array_empty_words, $body);
        $occurrences_body_word = array_count_values($tab_word_body);
        // count occurrences
        $doc->occurrences = sum_weight_head_and_body($occurrences_head_word, $occurrences_body_word);
    ...
    */
    function sum_weight_head_and_body($occurrences_head_word, $occurrences_body_word){
        $sum_weight_word = [];
        foreach ($occurrences_head_word as $word => $value) {
            $sum_weight_word[$word] = 1.5 * $value;
        }
    ...
    */
    function explode_text_in_html_file($separator, $array_empty_words, $text){
    ...
    */
    ...
    function main(){
        ...
        $HTML_FILES = index_multiple_html_files('../../FilesHtml', $array_empty_words, $separator);
        $conn =  configMysql();
